...
const stripOuterMarkdownFence = (value: string) => {
  ...
  const match = trimmed.match(/^```(?:\s*markdown|\s*md|\s*)\s*\r?\n([\s\S]*?)\r?\n```$/i);
  ...
  return (
    <div className="w-full h-full overflow-auto rounded-xl bg-slate-950/40 border border-indigo-500/20 px-4 py-3">
      <ReactMarkdown
        ...
        components={{
          h1: props => <h1 className="text-indigo-200 text-base font-bold mb-2" {...props} />,
          h2: props => <h2 className="text-indigo-200 text-sm font-bold mt-3 mb-2" {...props} />,
          h3: props => <h3 className="text-indigo-200 text-sm font-semibold mt-3 mb-2" {...props} />,
          p: props => <p className="text-indigo-100/90 text-sm leading-relaxed mb-2" {...props} />,
          ul: props => <ul className="list-disc ml-5 text-indigo-100/90 text-sm space-y-1 mb-2" {...props} />,
          ol: props => <ol className="list-decimal ml-5 text-indigo-100/90 text-sm space-y-1 mb-2" {...props} />,
          li: props => <li className="leading-relaxed" {...props} />,
          strong: props => <strong className="text-indigo-100 font-semibold" {...props} />,
          em: props => <em className="text-indigo-100/90 italic" {...props} />,
          code: props => <code className="font-mono text-[12px] bg-indigo-500/10 border border-indigo-500/20 px-1.5 py-0.5 rounded" {...props} />,
          pre: props => <pre className="overflow-auto rounded-lg bg-slate-950 border border-indigo-500/20 p-3 text-indigo-100/90 text-xs" {...props} />,
          ...
          tbody: props => <tbody className="divide-y divide-indigo-500/10" {...props} />,
...
const ProductCard = ({ product, onOpenGeo }: { product: Product, onOpenGeo: (p: Product) => void }) => {
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setGeoPageIndex(prev => (prev > 0 ? prev - 1 : geoList.length - 1));
                }}
              ...
              <button
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setGeoPageIndex(prev => (prev < geoList.length - 1 ? prev + 1 : 0));
                }}
                className="p-1.5 hover:bg-indigo-500/20 rounded-lg text-indigo-400 transition-colors"
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider shadow-sm bg-white/90 backdrop-blur-sm text-gray-700">
            {product.category === 'parametric' ? '数码电子' :
             product.category === 'scenario' ? '美妆护肤' : '母婴用品'}
          </span>
        </div>
      </div>
        ...
        {!isAiPerspective ? (
          <>
            <div className="flex items-center mb-2">
              ...
              <span className="text-xs text-gray-500">{product.reviewCount} 条评价</span>
            </div>
          ...
          <div className="mt-2 space-y-3 flex-1">
            <div className="space-y-1">
              <div className="text-[10px] font-mono text-indigo-400/70 uppercase">Core Functions</div>
              <div className="flex flex-wrap gap-1">
                {currentGeo.coreFunctions.slice(0, 2).map((f, i) => (
                  <span key={i} className="text-[9px] px-1.5 py-0.5 bg-indigo-500/10 border border-indigo-500/20 rounded text-indigo-300">
                    {f}
                  </span>
                ))}
              </div>
            ...
            <div className="space-y-1">
              <div className="text-[10px] font-mono text-indigo-400/70 uppercase">Conclusion</div>
            ...
            <div className="pt-4 mt-auto flex justify-between items-center border-t border-indigo-500/20">
              <span className="text-lg font-mono font-bold text-indigo-400">VAL: {product.price}</span>
              <div className="flex items-center gap-2">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    setGeoPageIndex(prev => (prev > 0 ? prev - 1 : (Array.isArray(product.geoOptimized) ? product.geoOptimized.length - 1 : 0)));
                  }}
                ...
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    setGeoPageIndex(prev => (prev < (Array.isArray(product.geoOptimized) ? product.geoOptimized.length - 1 : 0) ? prev + 1 : 0));
                  }}
                  className="p-1 hover:bg-indigo-500/20 rounded text-indigo-400"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth={2} /></svg>
...
const GeoModal = ({ product, onClose }: { product: Product | null, onClose: () => void }) => {
      ...
      <div className="max-w-4xl w-full bg-slate-900 border border-indigo-500/40 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-indigo-500/20 flex justify-between items-center bg-indigo-500/5">
          <div>
            ...
            <p className="text-xs font-mono text-indigo-500/60 mt-1">VERSION {pageIndex + 1} OF {geoList.length} | SOURCE: AI_GENERATED</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-indigo-500/20 rounded-full text-indigo-400 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 font-mono">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <section>
                <h4 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-3 border-l-2 border-indigo-500 pl-2">Product Identity</h4>
                <div className="bg-slate-800/50 p-4 rounded-xl border border-indigo-500/10">
                  <div className="text-sm text-indigo-100 mb-1"><span className="text-indigo-500/50 mr-2">NAME:</span> {current.productName}</div>
                  <div className="text-sm text-indigo-100"><span className="text-indigo-500/50 mr-2">TYPE:</span> {current.productType}</div>
                </div>
              </section>

              <section>
                <h4 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-3 border-l-2 border-indigo-500 pl-2">Core Functions</h4>
                <ul className="space-y-2">
                  {current.coreFunctions.map((f, i) => (
                    <li key={i} className="flex items-start text-sm text-indigo-200">
            ...
            <div className="space-y-6">
              <section>
                <h4 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-3 border-l-2 border-indigo-500 pl-2">Target Audience</h4>
              ...
              <section>
                <h4 className="text-xs font-bold text-red-500 uppercase tracking-widest mb-3 border-l-2 border-red-500 pl-2">Unsuitable Scenarios</h4>
                <ul className="space-y-2">
                  {current.unsuitableScenarios.map((s, i) => (
                    <li key={i} className="flex items-start text-sm text-red-300/80">
          ...
          <section className="pt-6 border-t border-indigo-500/20">
            <h4 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-4">Key Conclusion</h4>
            <div className="h-72">
...
const GeoControlCenter = () => {
  const { isAiPerspective, setIsAiPerspective, startGeneration, isGenerating, generationProgress, products, models } = useGeo();
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 border border-indigo-500/30 rounded-2xl p-8 text-center shadow-2xl shadow-indigo-500/20">
          ...
          <h3 className="text-xl font-mono font-bold text-white mb-2 tracking-tight">正在进行 GEO 优化生成...</h3>
          <p className="text-indigo-400/60 text-sm font-mono mb-6">正在调用 {engine} 并发处理 {products.length * Math.max(1, Math.min(count, 5))} 个内容节点</p>
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full overflow-hidden shadow-2xl animate-fadeIn">
            <div className="bg-slate-900 p-6 text-white">
              ...
              <p className="text-slate-400 text-xs mt-1">配置 AI 引擎以生成强结构化内容</p>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-2">生成数量 (每商品)</label>
                <input 
                ...
                <div className="flex justify-between mt-2 text-xs font-bold text-gray-600">
                  <span>1 个版本</span>
                  <span className="text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">{count} 个版本</span>
                  <span>5 个版本</span>
                </div>
              </div>
...
const HomePage = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'parametric' | 'scenario' | 'constraint'>('all');
  const [selectedGeoProduct, setSelectedGeoProduct] = useState<Product | null>(null);

  const uniqueProducts = useMemo(() => dedupeProductsById(products), [products]);
...
const ProductDetailPage = () => {
  ...
  if (!product) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => navigate('/')} className="mt-4 text-indigo-600">返回首页</button>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isAiPerspective ? 'bg-slate-950 text-indigo-100' : 'bg-white text-gray-900'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <button 
          onClick={() => navigate(-1)}
          className={`mb-8 flex items-center transition-colors ${isAiPerspective ? 'text-indigo-400 hover:text-indigo-200' : 'text-gray-500 hover:text-gray-900'}`}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <div className={`aspect-square rounded-2xl overflow-hidden border transition-all ${
              isAiPerspective ? 'bg-slate-900 border-indigo-500/30 grayscale opacity-70' : 'bg-gray-100 border-gray-200'
            }`}>
          ...
          {/* Product Info */}
          <div className="flex flex-col">
            <nav className={`flex mb-4 text-sm ${isAiPerspective ? 'text-indigo-500/60 font-mono' : 'text-gray-500'}`}>
              <span>首页</span>
              <span className="mx-2">/</span>
              <span>{product.category === 'parametric' ? '数码电子' : product.category === 'scenario' ? '美妆护肤' : '母婴用品'}</span>
            </nav>

            <h1 className={`text-3xl font-extrabold mb-2 ${isAiPerspective ? 'font-mono tracking-tight text-indigo-300' : 'text-gray-900'}`}>
              {isAiPerspective ? `[NODE_ID: ${product.id}] ${product.name}` : product.name}
            </h1>
            ...
            {!isAiPerspective ? (
              <>
                <div className="flex items-center mb-6">
                  <StarRating rating={product.rating} />
                  <span className="ml-4 text-sm text-indigo-600 font-medium">{product.reviewCount} 条真实评价</span>
                </div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl mb-8">
                  ...
                  <p className="mt-2 text-sm text-green-600 font-medium">库存充足 ({product.stock} 件)</p>
                </div>

                <div className="space-y-6 mb-8">
                  <div>
                    <h3 className="text-sm font-bold text-gray-900 mb-2">核心亮点</h3>
              ...
              <div className="mt-4 space-y-8 animate-fadeIn">
                <div className="bg-indigo-950/30 border border-indigo-500/20 p-6 rounded-2xl font-mono">
                  <div className="flex justify-between items-center mb-4 border-b border-indigo-500/20 pb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-indigo-500">GEO_STRUCTURED_DATA</span>
                      <span className="text-[10px] px-1 bg-indigo-500/20 rounded text-indigo-400">V{geoPageIndex + 1}</span>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setGeoPageIndex(p => Math.max(0, p - 1))} className="p-1 hover:bg-indigo-500/20 rounded text-indigo-400 disabled:opacity-20" disabled={geoPageIndex === 0}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" strokeWidth={2} /></svg>
                      </button>
                      <button onClick={() => setGeoPageIndex(p => Math.min(geoList.length - 1, p + 1))} className="p-1 hover:bg-indigo-500/20 rounded text-indigo-400 disabled:opacity-20" disabled={geoPageIndex === geoList.length - 1}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth={2} /></svg>
                      </button>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <section>
                      <p className="text-[10px] text-indigo-500/60 uppercase">Product Name</p>
                      <p className="text-sm text-indigo-200">{currentGeo.productName}</p>
                    </section>
                    <section>
                      <p className="text-[10px] text-indigo-500/60 uppercase">Product Type</p>
                      <p className="text-sm text-indigo-200">{currentGeo.productType}</p>
                    </section>
                    <section>
                      <p className="text-[10px] text-indigo-500/60 uppercase">Core Functions</p>
                      <ul className="text-sm text-indigo-200 list-disc list-inside">
                        {currentGeo.coreFunctions.map((f, i) => <li key={i}>{f}</li>)}
                      </ul>
                    </section>
                  </div>
                </div>
                ...
                <div className="bg-slate-900 border border-indigo-500/20 p-6 rounded-2xl font-mono">
                  <p className="text-[10px] text-indigo-500/60 uppercase mb-2">Key Conclusion</p>
                  <div className="h-48">
                    <MarkdownBox content={currentGeo.keyConclusion} />
                  </div>
                </div>
            ...
            <div className="flex gap-4 mt-auto pt-8">
              <button className={`flex-1 py-4 rounded-xl font-bold transition-all shadow-lg ${
                isAiPerspective 
              ...
              <button className={`flex-1 border-2 py-4 rounded-xl font-bold transition-all ${
                isAiPerspective 
        ...
        {/* Tabs Section */}
        <div className="mt-20">
          <div className={`border-b ${isAiPerspective ? 'border-indigo-500/20' : 'border-gray-200'}`}>
            <nav className="flex space-x-8">
              <button
                onClick={() => setActiveTab('details')}
                className={`py-4 px-1 text-sm font-bold border-b-2 ${
                  activeTab === 'details'
                    ? (isAiPerspective ? 'border-indigo-500 text-indigo-400' : 'border-indigo-600 text-indigo-600')
                    : 'border-transparent'
                }`}
              >
              ...
              <button
                ...
                className={`py-4 px-1 text-sm font-medium border-b-2 ${
                  activeTab === 'reviews'
                    ? (isAiPerspective ? 'border-indigo-500 text-indigo-400' : 'border-indigo-600 text-indigo-600')
                    : `border-transparent ${isAiPerspective ? 'text-indigo-500/40 hover:text-indigo-400' : 'text-gray-500 hover:text-gray-700'}`
                }`}
              >
                {isAiPerspective ? 'USER_FEEDBACK' : `用户评价 (${product.reviewCount})`}
              </button>
            </nav>
          </div>
          ...
          <div className="py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-8">
              {activeTab === 'details' ? (
                <>
                  <section>
                    <h2 className={`text-xl font-bold mb-4 ${isAiPerspective ? 'text-indigo-300 font-mono' : 'text-gray-900'}`}>
                      {isAiPerspective ? '>> DESCRIPTION_STREAM' : '产品介绍'}
                    </h2>
                    <p className={`leading-relaxed whitespace-pre-line ${isAiPerspective ? 'text-indigo-400/80 font-mono text-sm' : 'text-gray-600'}`}>
                  ...
                  {isAiPerspective && (
                    <section className="bg-indigo-950/20 border border-indigo-500/10 p-8 rounded-3xl font-mono">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                          <p className="text-[10px] text-indigo-500/60 uppercase mb-3">Target Audience</p>
                          <div className="flex flex-wrap gap-2">
                            {currentGeo.targetAudience.map((t, i) => (
                              <span key={i} className="bg-indigo-500/10 border border-indigo-500/30 px-3 py-1 rounded-full text-xs text-indigo-300">
                                {t}
                              </span>
                            ))}
                          </div>
                        ...
                        <div>
                          <p className="text-[10px] text-red-500/60 uppercase mb-3">Unsuitable Scenarios</p>
                          <div className="flex flex-wrap gap-2">
                            {currentGeo.unsuitableScenarios.map((t, i) => (
                              <span key={i} className="bg-red-500/10 border border-red-500/30 px-3 py-1 rounded-full text-xs text-red-300">
                                {t}
                              </span>
                            ))}
                          </div>
                ...
                <section>
                  ...
                  {product.reviews?.length ? (
                      ...
                      {product.reviews.map(r => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              <div className={`text-sm font-bold ${isAiPerspective ? 'font-mono text-indigo-200' : ''}`}>{r.user}</div>
                              <div className={`text-xs mt-1 ${isAiPerspective ? 'text-indigo-500/60 font-mono' : 'text-gray-400'}`}>{r.date}</div>
                            </div>
                            <StarRating rating={r.rating} />
                          </div>
                          <p className={`mt-4 text-sm leading-relaxed ${isAiPerspective ? 'text-indigo-200/90 font-mono' : 'text-gray-600'}`}>
                            {r.comment}
                          </p>
                        </div>
                      ))}
                    ...
                    <div className={`p-6 rounded-2xl border ${isAiPerspective ? 'bg-slate-900 border-indigo-500/20 text-indigo-400/70 font-mono' : 'bg-white border-gray-100 text-gray-500'}`}>
                      暂无评价
                    </div>
                  )}
                </section>
            ...
            <div className="lg:col-span-1">
              <div className={`p-6 rounded-2xl sticky top-24 border transition-all ${
                isAiPerspective ? 'bg-slate-900 border-indigo-500/20' : 'bg-gray-50 border-transparent'
              }`}>
                <h3 className={`font-bold mb-4 ${isAiPerspective ? 'text-indigo-300 font-mono' : 'text-gray-900'}`}>
                ...
                <ul className="space-y-4">
                  {[
                    ...
                    <li key={i} className="flex items-start">
                      <div className={`p-1.5 rounded-lg shadow-sm mr-3 ${isAiPerspective ? 'bg-indigo-600/20' : 'bg-white'}`}>
                        <svg className={`w-4 h-4 ${isAiPerspective ? 'text-indigo-400' : 'text-indigo-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                      </div>
                      <div>
                        <p className={`text-sm font-bold ${isAiPerspective ? 'text-indigo-200' : 'text-gray-900'}`}>{item.title}</p>
                        <p className={`text-xs ${isAiPerspective ? 'text-indigo-500/60' : 'text-gray-500'}`}>{item.desc}</p>
                      </div>
                    </li>
                  ))}
                </ul>
...
const App = () => {
  ...
  const startGeneration = async (modelId: string, count: number) => {
    ...
    const allCopyTypes: GeoCopyType[] = ['definition', 'problem', 'comparison', 'mechanism', 'boundary'];
    const safeCount = Math.max(1, Math.min(count, allCopyTypes.length));

    // 为每个商品独立随机选择 types
    const productCopyTypes: GeoCopyType[][] = products.map(() => shuffle(allCopyTypes).slice(0, safeCount));

    const totalTasks = products.length * safeCount;
    let completedTasks = 0;

    const results: any[][] = Array.from({ length: products.length }, () => Array(safeCount).fill(null));
    ...
    const updatedProducts = products.map((product, productIndex) => {
      ...
      return {
        ...product,
        geoOptimized: (newGeoOptimized.length ? newGeoOptimized : product.geoOptimized) as any
      };
    });
