...
import { AVAILABLE_MODELS, type GeoCopyType, mockProducts } from '@geo/shared-types';
...
export function registerApiRoutes(app: any) {
  return app
    ...
    .get('/api/products/:id', ({ params: { id }, set }: { params: { id: string }; set: any }) => {
    ...
    .post(
      '/api/generate',
      async ({ body, set }: { body: { productId: string; copyType?: GeoCopyType; model: string; customPrompt?: string }; set: any }) => {
        ...
        try {
          ...
          log('info', 'LLM generation ok', { productId, copyType, model, provider: selectedModel.provider, durationMs });
      ...
      },
      {
        body: t.Object({
          productId: t.String(),
          copyType: t.Optional(
            t.Union([t.Literal('definition'), t.Literal('problem'), t.Literal('comparison'), t.Literal('mechanism'), t.Literal('boundary')])
          ),
          model: t.String(),
