...
export const registerMonitorRoutes = (app: any) =>
    app.post(
           '/monitor/tasks',
           async (
               {body, set}: {body: Partial<TaskCreateRequest>; set: any}) => {
        ...
        .get(
            ...
            async ({
              ...
              const include_screenshot =
                  parseBoolean(query?.include_screenshot ?? true);
        ...
        .get('/monitor/health', async ({set}: {set: any}) => {
          const response = await gptInteractionApi.checkHealth();
          set.status = response.status || 502;
          return response.data;
        });
